...
class Rekapitulasi_model extends CI_Model
{
	...
	function getParameter($keyword=""){
		...
		$this->db->group_start();
		$this->db->like('field_label', $keyword);
		$this->db->group_end();
		return $this->db->get('parameter')->result();
	}

	function getDataHarian($limit=0, $mulai=1, $tglawal,$tglakhir,$subquery){
		return $this->db->select("date_format(`tgl_kunjungan`,'%Y-%m-%d') as `tgl_kunjungan`,$subquery
		COUNT(idx)AS jmlpasien")
		->where("date_format(`tgl_kunjungan`,'%Y-%m-%d') >=",$tglawal)
		->where("date_format(`tgl_kunjungan`,'%Y-%m-%d') <=",$tglakhir)
		->group_by("DATE_FORMAT(`tgl_kunjungan`,'%Y-%m-%d')")
		->limit($limit, $mulai)
		->get('pendaftaran')->result();
	}
	function getDataBulanan($limit=0, $mulai=1, $tglawal,$tglakhir,$subquery){
		return $this->db->select("date_format(`tgl_kunjungan`,'%M %Y') as `bulan_kunjungan`,$subquery
		COUNT(idx)AS jmlpasien")
		->where("date_format(`tgl_kunjungan`,'%Y%m') >=",$tglawal)
		->where("date_format(`tgl_kunjungan`,'%Y%m') <=",$tglakhir)
		->group_by("DATE_FORMAT(`tgl_kunjungan`,'%Y%m')")
		->limit($limit, $mulai)
		->get('pendaftaran')->result();
	}
	function getDataTahunan($limit=0, $mulai=1, $tglawal,$tglakhir,$subquery){
		return $this->db->select("date_format(`tgl_kunjungan`,'%Y') as `tahun_kunjungan`,$subquery
		COUNT(idx)AS jmlpasien")
		->where("date_format(`tgl_kunjungan`,'%Y') >=",$tglawal)
		->where("date_format(`tgl_kunjungan`,'%Y') <=",$tglakhir)
		->group_by("DATE_FORMAT(`tgl_kunjungan`,'%Y')")
		->limit($limit, $mulai)
		->get('pendaftaran')->result();
	}
	function getPekerjaan(){
		return $this->db->select('pekerjaan as pekerjaan_nama')->group_by('pekerjaan')->get('pendaftaran')->result();
	}
	
	function countrekapitulasi($keyword = "", $param = "")
	{
	...
	function getrekapitulasi($limit = 0, $mulai = 1, $keyword = "", $param = "")
	{
	...
	function getrekapitulasiBynomr($nomr)
	{
		$this->db->where('nomr', $nomr);
		return $this->db->get('rekapitulasi')->row();
	}
	...
	{
		$this->db->select('id_jenis_peserta,jenis_peserta');
		$this->db->group_by('id_jenis_peserta');
	...
	function cekKunjungan($id_poli,$nomr_rekapitulasi){
		$this->db->where('id_poli',$id_poli);
		$this->db->where('nomr_rekapitulasi',$nomr_rekapitulasi);
	...
	function updaterekapitulasi($data,$id){
		$this->db->where('idx',$id);
		$this->db->update('rekapitulasi',$data);
	}
	function insertrekapitulasi($data){
		$this->db->insert('rekapitulasi',$data);
		return $this->db->insert_id();
	}
	function getPendaftaran($id){
		$this->db->where('a.idx',$id);
		$this->db->join('rekapitulasi b','a.idx_rekapitulasi=b.idx');
		return $this->db->get('pendaftaran a')->row();
	}
	function generateNomr(){
		$query = $this->db->query("SELECT MAX(CONCAT(SUBSTR(nomr,5,2),SUBSTR(nomr,3,2),SUBSTR(nomr,1,2))) as max_nomr FROM rekapitulasi");
        $row = $query->row_array();
        ...
        // if ($cek == true) {
        //     $this->db->query("UPDATE rekapitulasi SET arc=0 WHERE nomr='$maxkode_nomr'");
        //     $nomr = $this->get_nomr();
        //     return $nomr;
        // } else {
        //     return $maxkode_nomr;
