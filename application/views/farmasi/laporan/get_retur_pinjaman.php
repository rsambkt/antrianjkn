...
<tr>
    ...
    <td width="5%">:</td>
    <td width="25%"><?php echo $x['KDBMK_RET'] ?></td>
	<td width="5%">&nbsp;</td>
    <td width="15%">Tgl Pembelian</td>
    <td width="5%">:</td>
    <td width="24%"><?php echo $x['DTBMK_RET'] ?></td>
</tr>
<tr>
    <td>No Faktur</td>
    <td>:</td>
...
<?php 
	...
	$subTotal = 0;
	$totalR = 0;
	$list = $this->db->select('b.*,a.HMODAL,a.JMLRET,c.NMSATUAN')
		  ->from('tbl04_barang_masuk_khusus_batal_detail a')
		  ->join('tbl04_barang b','a.KDBRG=b.KDBRG','LEFT')
		  ->join('tbl04_satuan c','b.KDSATUAN=c.KDSATUAN','LEFT')
		  ->where('KDBMK_RET',$x['KDBMK_RET'])
		  ->order_by('NMBRG', 'ASC')
		  ->get();
