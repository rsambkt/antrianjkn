            ...
            <div class="box box-success">
                ...
                <div class="box-body table-responsive">
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th width="120px">Waktu Rekam</th>
                                <th>Nama Rekanan</th>
                                <th width="120px">No Bukti</th>
                                <th width="100px">Tgl Bukti</th>
                    ...
                    <div class="row-fluid" style="margin-top: -10px">
                        <form id="form1" action="#" method="get" class="form-horizontal">
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <input type="text" name="KDBMK" id="KDBMK" class="form-control"/>
                                </div>
                            </div>
                            <div class="control-group">
...
$(document).ready(function(){
    ...
    $('#btnKembali').click(function(){
        var url = '<?php echo base_url().'farmasi/barang_masuk_khusus' ?>';
        window.location.href = url;        
    });
    $('#btnTambah').click(function(){
        //var a = "<?php echo $kLok ?>";
        var url = "<?php echo base_url().'farmasi/barang_masuk_khusus/tambah' ?>";
        window.location.href = url;        
    });
    ...
    $('#keywordFilter').change(function(){
        ...
        $.ajax({
            url : "<?php echo base_url().'farmasi/barang_masuk_khusus/getView' ?>",
            type : "POST",
            ...
            success : function(data){
                ...
                $('tr.resultDat').each(function(){
                    var a = $(this).data('id');
                    $.ajax({
                        url         : "<?php echo base_url().'farmasi/barang_masuk_khusus/cekRetur' ?>",
                        type        : "POST",
                        data        : {KDBMK:a},
                        dataType    : "JSON",
                        success     : function(data){
                            if (data.code==200){
                                $('button#'+a).parent('.btnAksi').append('<br/>Transaksi di Retur');
                                $('button#'+a).remove();
                            }
                        },
...
function cetak(a){
    window.open('<?php echo base_url().'farmasi/barang_masuk_khusus/cetak?kode=' ?>'+a);
} 
function batal(a,b){
...
function submitRetur(){
    ...
    var b = $('#ALASAN').val();
    $.ajax({
        url         : "<?php echo base_url().'farmasi/barang_masuk_khusus/batalRecord' ?>",
        type        : "POST",
        data        : {KDBMK:a,ALASAN:b},
        dataType    : "JSON",
        success     : function(data){
            if(data.code==401){
                alert(data.message);
