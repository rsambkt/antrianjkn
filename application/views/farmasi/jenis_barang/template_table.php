                ...
                <div class="box-header with-border">
                    ...
                    <div class="box-tools">
                        <div class="input-group input-group-sm" style="width: 200px;">
                            <input type="text" id="keyword" name="keyword" class="form-control pull-right" placeholder="Enter Jenis Barang" style="width: 200px" />
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Form Jenis Barang</h4>
            </div>
                    ...
                    <div class="box-body">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="hidden" name="KDJENISBRG" id="KDJENISBRG">
                                <input type="hidden" class="csrf" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>">
                                <input type="text" class="form-control" name="JENISBRG" id="JENISBRG">
                            </div>
                        </div>
                    </div>
                </form>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#keyword').keypress(function(ev) {
            ...
            if (keycode == '13') {
                $.ajax({
                    url: "<?php echo base_url() . 'farmasi/jenis_barang/getView' ?>",
                    type: "POST",
                    data: {
                        sLike: $(this).val(),
    ...
    function hapus(a) {
        ...
        if (x) {
            $.ajax({
                url: "<?php echo base_url() . 'farmasi/jenis_barang/hapus' ?>",
                type: "POST",
                data: {
                    KDJENISBRG: a,
                    <?= $this->security->get_csrf_token_name(); ?>: $('#csrf').val()
    ...
    function simpan() {
        var a = $('#JENISBRG').val();
        if (a == "") {
            alert('Ops. Jenis Barang harus di isi.');
        } else {
            $.ajax({
                url: "<?php echo base_url() . 'farmasi/jenis_barang/simpan' ?>",
                type: "POST",
