...
<body>
    ...
    foreach ($dataPreview->result_array() as $x) :
        $ap = explode(',', $x['AP']);
        //print_r($ap);
        $SAT = explode('#', $x['AP_SATUAN']);
        //print_r($SAT);
        //if (count($SAT) > 1) $NMSATUAN = $SAT[1];
        //else $NMSATUAN = $this->farmasi_model->getField('NMSATUAN', 'tbl04_barang', 'KDBRG', $x["KDBRG"]);
    ...
    ?>
        <div id="page_print">
            ...
            <div class="font7"><?php echo date('d/m/Y', strtotime($tgl_lahir)) . " (" . $umur->y . " Thn " . $umur->m . " Bulan)" ?></div>
            ...
            if ($JNS_RESEP == "Resep Harian") {
                $brg = $this->farmasi_model->getListBarangByAp($x["KDJL"], $x["AP"]);
                $no = 0;
                foreach ($brg as $b) {
                    $no++;
                    $NMSATUAN = $this->farmasi_model->getField('NMSATUAN', 'tbl04_barang', 'KDBRG', $x["KDBRG"]);
                    echo "<div class='row font7'>" . $no . ". " . $b->NMBRG . " " . $b->JMLJUAL . " " . $NMSATUAN . "</div>";
                }
            ?>
                <div class="row tebal font9" style="padding: 2px 0px; text-align:left"><?php echo $x["AP"]; ?></div>
            ...
            ?>
                ...
                <div class="tebal font10"><?php echo @$ap[1] ?></div>
                <div class="tebal font10"><?= @$ap[2] ?></div>
