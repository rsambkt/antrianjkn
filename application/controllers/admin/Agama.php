...
class Agama extends CI_Controller
{
    ...
    function index()
    {
        if ($this->session->userdata('modul')==1) {
            ...
            $config = array(
                'url'           => 'admin/agama/dataagama',
                'variable'      => array('idx'=>'idx','agama' => 'agama'),
                'field'         => array('agama'),
                'function'      => 'getagama',
                'keyword_id'    => 'q',
                'param_id'      => 'param',
            ...
            $data = array('contentTitle' => 'Agama', 'config' => $config);
            $view = array(
                'header' => $this->load->view('template/header', '', true),
                'nav_sidebar' => $this->load->view('template/nav_sidebar', array(), true),
                'content' => $this->load->view('admin/master/agama', $data, true),
                'index_menu'=>2,
                'lib'           => array('javascript/agama.js'),
    ...
    function dataagama()
    {
        if ($this->session->userdata('modul')==1) {
            ...
            $response = array(
                ...
                'message'   => "OK",
                'start'     => $mulai,
                'row_count' => $this->agama_model->countagama($keyword, $param),
                'limit'     => $limit,
                'data'      => $this->agama_model->getagama($limit, $mulai, $keyword, $param),
            );
        } else {
    ...
    function insert(){
        if ($this->session->userdata('modul')==1) {
            ...
            if(empty($idx)) {
                $id=$this->agama_model->insertagama($data);
                $pesan="Agama berhasil disimpan";
            }
            else {
                $id=$this->agama_model->updateagama($data,$idx);
                $pesan="Agama berhasil diupdate";
            }
    ...
    function update(){
        if ($this->session->userdata('modul')==1) {
            ...
            $this->agama_model->updateagama($data,$idx);
            $response = array('status' => true, 'message' => 'Data agama berhasil diupdate');
        }else {
            $response = array('status' => false, 'message' => 'Session Expired');
        }
        header('Content-Type: application/json');
    ...
    function edit($idx){
        if ($this->session->userdata('modul')==1) {
            $data=$this->agama_model->getagamaById($idx);
    ...
    function hapus($idx){
        if ($this->session->userdata('modul')==1) {
            $data=$this->agama_model->hapusagama($idx);
