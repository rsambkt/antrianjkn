...
class Suku extends CI_Controller
{
    ...
    function index()
    {
        if ($this->session->userdata('modul')==1) {
            ...
            $config = array(
                'url'           => 'admin/suku/datasuku',
                'variable'      => array('idx'=>'idx','nama_suku' => 'nama_suku'),
                'field'         => array('nama_suku'),
                'function'      => 'getsuku',
                'keyword_id'    => 'q',
                'param_id'      => 'param',
                'limit_id'      => 'limit',
            ...
            $data = array('contentTitle' => 'suku', 'config' => $config);
            $view = array(
                'header' => $this->load->view('template/header', '', true),
                'nav_sidebar' => $this->load->view('template/nav_sidebar', array(), true),
                'content' => $this->load->view('admin/master/suku', $data, true),
                'index_menu'=>2,
                'lib'           => array('javascript/suku.js'),
    ...
    function datasuku()
    {
        if ($this->session->userdata('modul')==1) {
            ...
            $response = array(
                ...
                'message'   => "OK",
                'start'     => $mulai,
                'row_count' => $this->suku_model->countsuku($keyword, $param),
                'limit'     => $limit,
                'data'      => $this->suku_model->getsuku($limit, $mulai, $keyword, $param),
            );
        } else {
    ...
    function insert(){
        if ($this->session->userdata('modul')==1) {
            ...
            if(empty($idx)) {
                $id=$this->suku_model->insertsuku($data);
                $pesan="suku berhasil disimpan";
            }
            else {
                $id=$this->suku_model->updatesuku($data,$idx);
                $pesan="suku berhasil diupdate";
            }
    ...
    function update(){
        if ($this->session->userdata('modul')==1) {
            ...
            $this->suku_model->updatesuku($data,$idx);
            $response = array('status' => true, 'message' => 'Data suku berhasil diupdate');
        }else {
            $response = array('status' => false, 'message' => 'Session Expired');
        }
        header('Content-Type: application/json');
    ...
    function edit($idx){
        if ($this->session->userdata('modul')==1) {
            $data=$this->suku_model->getsukuById($idx);
    ...
    function hapus($idx){
        if ($this->session->userdata('modul')==1) {
            $data=$this->suku_model->hapussuku($idx);
