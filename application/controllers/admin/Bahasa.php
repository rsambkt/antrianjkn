...
class Bahasa extends CI_Controller
{
    ...
    function index()
    {
        if ($this->session->userdata('modul')==1) {
            ...
            $config = array(
                'url'           => 'admin/bahasa/databahasa',
                'variable'      => array('idx'=>'idx','bahasa' => 'bahasa'),
                'field'         => array('bahasa'),
                'function'      => 'getbahasa',
                'keyword_id'    => 'q',
                'param_id'      => 'param',
            ...
            $data = array('contentTitle' => 'bahasa', 'config' => $config);
            $view = array(
                'header' => $this->load->view('template/header', '', true),
                'nav_sidebar' => $this->load->view('template/nav_sidebar', array(), true),
                'content' => $this->load->view('admin/master/bahasa', $data, true),
                'index_menu'=>2,
                'lib'           => array('javascript/bahasa.js'),
    ...
    function databahasa()
    {
        if ($this->session->userdata('modul')==1) {
            ...
            $response = array(
                ...
                'message'   => "OK",
                'start'     => $mulai,
                'row_count' => $this->bahasa_model->countbahasa($keyword, $param),
                'limit'     => $limit,
                'data'      => $this->bahasa_model->getbahasa($limit, $mulai, $keyword, $param),
            );
        } else {
    ...
    function insert(){
        if ($this->session->userdata('modul')==1) {
            ...
            if(empty($idx)) {
                $id=$this->bahasa_model->insertbahasa($data);
                $pesan="bahasa berhasil disimpan";
            }
            else {
                $id=$this->bahasa_model->updatebahasa($data,$idx);
                $pesan="bahasa berhasil diupdate";
            }
    ...
    function update(){
        if ($this->session->userdata('modul')==1) {
            ...
            $this->bahasa_model->updatebahasa($data,$idx);
            $response = array('status' => true, 'message' => 'Data bahasa berhasil diupdate');
        }else {
            $response = array('status' => false, 'message' => 'Session Expired');
        }
        header('Content-Type: application/json');
    ...
    function edit($idx){
        if ($this->session->userdata('modul')==1) {
            $data=$this->bahasa_model->getbahasaById($idx);
    ...
    function hapus($idx){
        if ($this->session->userdata('modul')==1) {
            $data=$this->bahasa_model->hapusbahasa($idx);
