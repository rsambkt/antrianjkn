...
function simpan(){
    var url;
    url = base_url + "admin/users/insert";
...
function hapus(id) {
    ...
    if (isConfirm) {
        $.ajax({
            url: base_url + "admin/users/hapus/" + id,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
                if(data.status==true){
            ...
            error: function (jqXHR, textusers, errorThrown) {
                alert('Error')
            }
        });
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data.status==true){
                ...
                if(data.data.users==1) $('#users').prop('checked',true);
                else $('#users').prop('checked',false);
            }else{
